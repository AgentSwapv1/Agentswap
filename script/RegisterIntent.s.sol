...
contract RegisterIntent is Script {
    function run() external {
        ...
        bytes32 swapId =
            vm.envOr("TEST_SWAP_ID", bytes32(keccak256(abi.encodePacked("testswap", block.timestamp, block.number))));
        ...
        AgentSwapHook(hookAddr).registerIntent(swapId, feeOverride, mevFlag, evidence, ts, sig);
        vm.stopBroadcast();
